...
<configuration>
  <input>
      <net-file value="use.net.xml"/>   <!--路网文件-->
      <route-files value="use.rou.xml,fl_bus.flow.xml,fl_person.add.xml,after.rou.xml,fl_train.flow.xml,fl_ParkingArea.flow.xml"/> <!--车辆路径、公交车流量、行人路径、停车场路径、轨道交通流量等-->
      <additional-files value="bus_def.add.xml,E3.add.xml,E2.add.xml,parking.add.xml,rail_station.add.xml"/> <!--附加文件：公交站设置、停车场设置、地铁站设置、检测器设置-->
  </input>

  <output>
      <write-license value="false"/>  <!--	Include license info into every output file; default: false -->
      <output-prefix value="TIME"/>  <!-- Prefix which is applied to all output files. The special string 'TIME' is replaced by the current time.-->
      ...
      <netstate-dump value="data_netstate.xml"/>  <!--Save complete network states into FILE -->
      <emission-output value="data_emission.xml"/>  <!--Save the emission values of each vehicle -->
      ...
      <queue-output value="data_Queue.xml"/>  <!--Save the vehicle queues at the junctions (experimental) -->
      <queue-output.period value="300"/>  <!-- Save vehicle queues with the given period; default: -1-->
      <summary-output value="data_summary.xml"/>  <!-- Save aggregated vehicle departure info into FILE-->
      <summary-output.period value="300"/>  <!-- Save summary-output with the given period; default: -1-->
      <person-summary-output value="data_person_summary.xml"/>  <!-- Save aggregated person counts into FILE-->
      <personinfo-output value="data_personinfo.xml"/>  <!-- Save personinfo and containerinfo to separate FILE-->
      <link-output  value="data_link.xml"/>  <!-- Save links states into FILE-->
      <collision-output value="data_collision.xml"/>  <!-- Write collision information into FILE-->
      <edgedata-output value="data_edges.xml"/>  <!-- Write aggregated traffic statistics for all edges into FILE-->
      <lanedata-output value="data_lanes.xml"/>  <!-- Write aggregated traffic statistics for all lanes into FILE-->
      <statistic-output value="data_statistic.xml"/>  <!-- Write overall statistics into FILE-->
      <save-state.files value="data_savestate.xml"/>  <!-- 	Files for network states-->
      <save-state.precision value="2"/>  <!-- Write internal state values with the given precision (default 2); default: 2-->
  ...
  <processing>
      <no-internal-links value="true"/>  <!-- Disable (junction) internal links; default: false-->
      <ignore-route-errors value="true"/>  <!-- Do not check whether routes are connected; default: false-->
      <ignore-accidents value="true"/>  <!-- Do not check whether accidents occur; default: false-->
  ...
  <gui_only>
    ...
    <osg-view value="false"/>  <!-- 	Start with an OpenSceneGraph view instead of the regular 2D view; default: false-->
